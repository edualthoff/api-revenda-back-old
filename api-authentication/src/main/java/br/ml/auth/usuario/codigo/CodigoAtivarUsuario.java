...
public class CodigoAtivarUsuario implements CodigoAtivar<Usuario> {
	...
	public CodigoAtivarUsuario(Usuario user) {
		super();
		this.user = user;
	}

	public CodigoAtivarUsuario() {
		super();
	}
	...
	public String gerarCodigo() {
		...
		codigo = user.getEmail()+separador+user.getId().toString()+separador+user.isVerificado()+separador+time;
		
		return Base64.getUrlEncoder().encodeToString(new Cifrador().criptografar(codigo));
	}
